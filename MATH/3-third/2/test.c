...
int find_norm(int count, Vector **max_norm, int *count_1, int *count_p, int *count_m, Vector **max_p_norm,
        ...
        if (cur_matrix_norm >= cur_max_matrix_norm) {
            if (cur_matrix_norm > cur_max_matrix_norm) {
                ...
                *max_matrix_norm = (Vector *)malloc(10 * sizeof(Vector));
                ...
                if (*count_m == k_) {
                    temp = (Vector *)realloc(*max_matrix_norm, 2 * (*count_m) * sizeof(Vector));
                    if (temp == NULL) {
                        return NO_MEMORY;
                    }
...
int main(int argc, char *argv[]) {
    ...
    int result = find_norm(3, &max_norm, &count_1, &count_p, &count_m, &max_p_norm, &max_A_norm, p, size,
