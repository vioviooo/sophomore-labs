...
int find_norm(int count, Vector **max_norm, int *cnt, int *cnt_p, int *cnt_m, Vector **max_p_norm,
              Vector **max_matrix_norm, int p, int size, double **matrix, double (*get_norm)(Vector),
              double (*p_norm)(Vector, double), double (*get_matrix_norm)(Vector, double **), ...) {
        ...
        if (my_p_norm >= cur_max_p_norm) {
            if (my_p_norm > cur_max_p_norm) {
                ...
                *max_p_norm = (Vector *)malloc(10 * sizeof(Vector));
                ...
                if (*cnt_p == i) {
                    temp = (Vector *)realloc(*max_p_norm, 2 * (*cnt_p) * sizeof(Vector));
                    if (temp == NULL) {
                        return NO_MEMORY;
                    }
                    *max_p_norm = temp;
        ...
        if (cur_matrix_norm >= cur_max_matrix_norm) {
            if (cur_matrix_norm > cur_max_matrix_norm) {
                ...
                *max_matrix_norm = (Vector *)malloc(10 * sizeof(Vector));
                ...
                if (*cnt_m == i) {
                    temp = (Vector *)realloc(*max_matrix_norm, 2 * (*cnt_m) * sizeof(Vector));
                    if (temp == NULL) {
                        return NO_MEMORY;
                    }
...
void clean_me(double **matrix, Vector *max_p_norm, Vector *max_norm, Vector *max_A_norm, int size) {
...
int main(int argc, char *argv[]) {
    ...
    int result = find_norm(vec_cnt, &max_norm, &cnt, &cnt_p, &cnt_m, &max_p_norm, &max_A_norm, p, size,
                           matrix, get_norm, p_norm, get_matrix_norm, vec1, vec2, vec3);
